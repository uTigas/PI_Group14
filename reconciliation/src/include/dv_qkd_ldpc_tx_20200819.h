...
#include "dv_qkd_ldpc_tx_error_correction_20200819.h"
...
public:
	...
	void setBypassErrorCorrection(t_bool bErrorCorrection) { DvQkdLdpcTxErrorCorrection_.setBypassErrorCorrection(bErrorCorrection); };
	...
	void setLdpcErrorCorrectionAlgorithMaxIterations(t_integer lErrorCorrectionAlgorithMaxIterations) { DvQkdLdpcTxErrorCorrection_.setLdpcErrorCorrectionAlgorithMaxIterations(lErrorCorrectionAlgorithMaxIterations); };
	t_integer getLdpcErrorCorrectionAlgorithMaxIterations(t_integer lErrorCorrectionAlgorithMaxIterations) const { DvQkdLdpcTxErrorCorrection_.getLdpcErrorCorrectionAlgorithMaxIterations(); };

	void setCodeLength(t_integer cLength) { DvQkdLdpcTxErrorCorrection_.setCodeLength(cLength); };
	t_integer getCodeLength(void) const { DvQkdLdpcTxErrorCorrection_.getCodeLength(); };

	void setCodeRank(t_integer cRank) { DvQkdLdpcTxErrorCorrection_.setCodeRank(cRank); };
	t_integer getCodeRank(void) const { DvQkdLdpcTxErrorCorrection_.getCodeRank(); };

	void setNumberOfNonZerosValuesInSparseParityCheckMatrix(t_integer nOfNonZerosValuesInSparseParityCheckMatrix) { DvQkdLdpcTxErrorCorrection_.setNumberOfNonZerosValuesInSparseParityCheckMatrix(nOfNonZerosValuesInSparseParityCheckMatrix); };
	t_integer getNumberOfNonZerosValuesInSparseParityCheckMatrix(void) const { DvQkdLdpcTxErrorCorrection_.getNumberOfNonZerosValuesInSparseParityCheckMatrix(); };

	void setNumberOfDataBitsPerBitFillingPeriod(t_integer nOfDataBitsPerBitFillingPeriod) { DvQkdLdpcTxErrorCorrection_.setNumberOfDataBitsPerBitFillingPeriod(nOfDataBitsPerBitFillingPeriod); }
	t_integer getNumberOfDataBitsPerBitFillingPeriod() const { return DvQkdLdpcTxErrorCorrection_.getNumberOfDataBitsPerBitFillingPeriod(); }
	
	void setNumberOfDummyBitsPerBitFillingPeriod(t_integer nOfDummyBitsPerBitFillingPeriod) { DvQkdLdpcTxErrorCorrection_.setNumberOfDummyBitsPerBitFillingPeriod(nOfDummyBitsPerBitFillingPeriod); }
	t_integer getNumberOfDummyBitsPerBitFillingPeriod() const { return DvQkdLdpcTxErrorCorrection_.getNumberOfDummyBitsPerBitFillingPeriod(); }
...
private:
	...
	DvQkdLdpcTxErrorCorrection DvQkdLdpcTxErrorCorrection_{ {&DvQkdLdpcTxParameterEstimation_DataOut, &SindromeFromRx, &PermutationsFromRx, &DvQkdLdpcTxParameterEstimation_EstimatedQBER, &ToggleBERChange}, {&DataCorr, &ParityToSend} };
