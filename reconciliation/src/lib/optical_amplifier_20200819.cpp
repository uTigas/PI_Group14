...
OpticalAmplifier::OpticalAmplifier(std::initializer_list<Signal *> inputSignal, std::initializer_list<Signal *> outputSignal):SuperBlock(inputSignal, outputSignal)
{
	setGain_dB(gain_dB);
...
void OpticalAmplifier::initialize()
{
	...
	WhiteNoise_.setSamplingPeriod(inputSignals[0]->getSamplingPeriod());
	WhiteNoise_.setSymbolPeriod(inputSignals[0]->getSymbolPeriod());
...
bool OpticalAmplifier::runBlock()
{
...
/*
void OpticalAmplifier::terminate()
{
	terminateSuperBlock();

	auto ready = OpticalSignal_Out.ready();
