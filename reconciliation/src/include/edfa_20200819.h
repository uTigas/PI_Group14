...
class EDFA : public SuperBlock 
{
	...
	EDFA(std::initializer_list<Signal*> inputSignal, std::initializer_list<Signal*> outputSignal);
	...
	void setGain_dB(t_real g)
	{
		...
		WhiteNoise_.setUnilateralNoiseSpectralDensity(PLANCK_CONSTANT * SPEED_OF_LIGHT * (1 / wavelength_m) * (pow(10, gain_dB / 10) - 1) * pow(10, noiseFigure_dB / 10) / 2);
	}
	...
	{
			wavelength_m = w_m;
			WhiteNoise_.setUnilateralNoiseSpectralDensity(PLANCK_CONSTANT*SPEED_OF_LIGHT*(1 / w_m)*(pow(10, gain_dB / 10) - 1) * pow(10, noiseFigure_dB / 10) / 2);
	}
