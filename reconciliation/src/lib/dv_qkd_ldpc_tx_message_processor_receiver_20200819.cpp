...
void DvQkdLdpcTxMessageProcessorReceiver::initialize(void)
{
	Sindrome.resize(outputSignals[5]->getBufferLength());
}

bool DvQkdLdpcTxMessageProcessorReceiver::runBlock(void)
{
	bool alive = false;
	...
	for (auto k = 0; k < ready; ++k)
	{
		...
		else if (mType == "SindromeMsg")
		{
			...
			if (space >= dataLength)
			{
				...
				inputSignals[0]->bufferGet();
				for (auto k = 0; k < dataLength - hashLength; k++)
				{
					t_binary value = (t_binary)(mDataString.at(k) - '0');
					Sindrome.bufferPut((t_binary)(value));
				}
				for (auto k = dataLength - hashLength; k < dataLength; k++) // hash
				{
					t_binary value = (t_binary)(mDataString.at(k));
					Sindrome.bufferPut((t_binary)(value));
				}
...
bool DvQkdLdpcTxMessageProcessorReceiver::outputReceivedData(std::vector<t_integer>& dataVector, Signal& outputSignal, bool& started)
{
	bool alive = false;
