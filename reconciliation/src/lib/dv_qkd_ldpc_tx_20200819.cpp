#include "dv_qkd_ldpc_tx_20200819.h"
...
void DvQkd_LDPC_Tx::initialize(void) 
{
	...
	DvQkdLdpcTxErrorCorrection_.setParityCheckMatrixFolderName(parityCheckMatrixFolderName);
	DvQkdLdpcTxErrorCorrection_.setParityCheckMatrixFileName(parityCheckMatrixFileName);
...
bool DvQkd_LDPC_Tx::runBlock(void) 
{
	...
	if ((dispBitsTracker >= dispBitsInterval) && (toPrint)) {
		...
		double ecBitsIn = (double)DvQkdLdpcTxErrorCorrection_.getTotalNumberOfInputBits();
		double ecBitsOut = (double)DvQkdLdpcTxErrorCorrection_.getTotalNumberOfOutputBits();

		uint64_t numberOfCalculatedSindromes = DvQkdLdpcTxErrorCorrection_.getNumberOfCalculatedSindromes();
		uint64_t numberOfNoMatchedSindromes = DvQkdLdpcTxErrorCorrection_.getNumberOfNoMatchedSindromes();
