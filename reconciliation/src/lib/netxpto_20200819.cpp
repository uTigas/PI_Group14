...
void t_handler_message::setDestinationBlock(t_string destinationBlock) {
	// Check given Destination Block to be the right size
	destinationBlock.resize(DST_BLOCK_SIZE, ' ');

	// Replace with new Destination Block
	rawMessage.replace(0, DST_BLOCK_SIZE, destinationBlock);
};

t_string t_handler_message::getDestinationBlock() const {
	t_string destinationBlock = rawMessage.substr(0, DST_BLOCK_SIZE);
	return destinationBlock.substr(0, destinationBlock.find(' '));
}

/* -------------------------------------------------------------------- */

void t_handler_message::setSourceBlock(t_string sourceBlock) {
	// Check given Source Block to be the right size
	sourceBlock.resize(SRC_BLOCK_SIZE, ' ');

	// Replace with new Source Block
	rawMessage.replace(DST_BLOCK_SIZE, SRC_BLOCK_SIZE, sourceBlock);
};

t_string t_handler_message::getSourceBlock() const {
	t_string sourceBlock =
		rawMessage.substr(DST_BLOCK_SIZE, SRC_BLOCK_SIZE);
	return sourceBlock.substr(0, sourceBlock.find(' '));
}

/* -------------------------------------------------------------------- */

void t_handler_message::setPriority(uint8_t priority) {
	// Check given priority (If the value isn't in the [0,5] interval, it'll be replace with 0)
	if (priority > 5)
		priority = 0;

	// Insert Priority in message
	uint32_t headerSizeUntilPriority = DST_BLOCK_SIZE + SRC_BLOCK_SIZE;
	rawMessage.replace(headerSizeUntilPriority, PRIORITY_SIZE,
...
/*
	...
	*/
uint8_t t_handler_message::getPriority() const {
	uint32_t headerSizeUntilPriority = DST_BLOCK_SIZE + SRC_BLOCK_SIZE;
	t_string priority =
		rawMessage.substr(headerSizeUntilPriority, PRIORITY_SIZE);
	return static_cast<uint8_t>(std::stoul(priority));
	;
}

/* -------------------------------------------------------------------- */

void t_handler_message::setSequentialId(uint32_t Id) {
	// Insert Id in message
	uint32_t headerSizeUntilId =
		DST_BLOCK_SIZE + SRC_BLOCK_SIZE + PRIORITY_SIZE;
	rawMessage.replace(headerSizeUntilId, ID_SIZE, std::to_string(Id));
};

//! Note that this function can launch an exception on converting from string to integer
uint32_t t_handler_message::getSequentialId() const {
	uint32_t headerSizeUntilId =
		DST_BLOCK_SIZE + SRC_BLOCK_SIZE + PRIORITY_SIZE;
	t_string Id = rawMessage.substr(headerSizeUntilId, ID_SIZE);
...
void t_handler_message::setMessageType(t_string mType) {
	// Check given message type to be the right size
	mType.resize(MSG_TYPE_SIZE, ' ');

	// Replace with new message type
	uint32_t headerSizeUntilMessageType =
		DST_BLOCK_SIZE + SRC_BLOCK_SIZE + PRIORITY_SIZE + ID_SIZE;
	rawMessage.replace(headerSizeUntilMessageType, MSG_TYPE_SIZE, mType);
};

t_string t_handler_message::getMessageType() const {
	uint32_t headerSizeUntilMessageType =
		DST_BLOCK_SIZE + SRC_BLOCK_SIZE + PRIORITY_SIZE + ID_SIZE;
	t_string messageType =
		rawMessage.substr(headerSizeUntilMessageType, MSG_TYPE_SIZE);
	return messageType.substr(0, messageType.find(' '));
}

/* -------------------------------------------------------------------- */

void t_handler_message::setSourcePort(uint8_t sourcePort) {
	// Insert source Port in message
	uint32_t headerSizeUntilSourcePort = DST_BLOCK_SIZE + SRC_BLOCK_SIZE +
											PRIORITY_SIZE + ID_SIZE +
											MSG_TYPE_SIZE;
	rawMessage.replace(headerSizeUntilSourcePort, SRC_PORT_SIZE,
						std::to_string(sourcePort));
};

//! Note that this function can launch an exception on converting from string to integer
uint8_t t_handler_message::getSourcePort() const {
	uint32_t headerSizeUntilSourcePort = DST_BLOCK_SIZE + SRC_BLOCK_SIZE +
											PRIORITY_SIZE + ID_SIZE +
											MSG_TYPE_SIZE;
	t_string sourcePort =
		rawMessage.substr(headerSizeUntilSourcePort, SRC_PORT_SIZE);
	return static_cast<uint8_t>(std::stoul(sourcePort));
	;
}

/* -------------------------------------------------------------------- */

void t_handler_message::setMessageDataLength(t_string mDataLength) {
	// Check if the given value is in the correct format
	if (!isNumber(mDataLength)) {
		throw std::invalid_argument(
			"Message data length must be a positive integer");
	}

	uint32_t dataLength = strToNumber(mDataLength);
	if (dataLength > getMaxDataSize()) {
		throw std::invalid_argument(
			"Message data length exceeds maximum allowed size");
	}

	// Format the data length string with leading zeros
	t_string aux;
	aux.resize(MSG_DATA_LENGTH_SIZE, '0');
	aux.replace(MSG_DATA_LENGTH_SIZE - mDataLength.size(),
				mDataLength.size(), mDataLength);

	// Replace the data length field in the raw message
	uint32_t startDataLength = HEADER_SIZE - MSG_DATA_LENGTH_SIZE;
	rawMessage.replace(startDataLength, MSG_DATA_LENGTH_SIZE, aux);
};

t_string t_handler_message::getMessageDataLength() const {
	t_string messageLength = rawMessage.substr(
		HEADER_SIZE - MSG_DATA_LENGTH_SIZE, MSG_DATA_LENGTH_SIZE);
	// Remove zeros at the start
	messageLength.erase(0, messageLength.find_first_not_of('0'));
	return messageLength;
}

/* -------------------------------------------------------------------- */

void t_handler_message::setMessageData(t_string mData) {
	if (mData.size() > strToNumber(getMessageDataLength()))
		mData.resize(strToNumber(getMessageDataLength()));
	rawMessage.resize(HEADER_SIZE);
	rawMessage.append(mData);
};

t_string t_handler_message::getMessageData() const { return rawMessage.substr(HEADER_SIZE); }

/* -------------------------------------------------------------------- */

void t_handler_message::setHeader(t_string destinationBlock, t_string sourceBlock,
	...
	setMessageDataLength(mDataLength);
}

bool t_handler_message::operator<(const t_handler_message& other) const {
	if (getPriority() != other.getPriority()) {
		return getPriority() < other.getPriority();
	}
	// If priorities are equal, order by insertion time (New element placed BEFORE old one)
	return false;
}

//!This function is used to asses if this and other class have equal values
bool t_handler_message::operator==(const t_handler_message& other) const {
	if (getDestinationBlock() != other.getDestinationBlock())
		return false;
	if (getSourceBlock() != other.getSourceBlock())
		return false;
	if (getPriority() != other.getPriority())
		return false;
	if (getSequentialId() != other.getSequentialId())
		return false;
	if (getMessageType() != other.getMessageType())
		return false;
	if (getSourcePort() != other.getSourcePort())
		return false;
	if (getMessageDataLength() != other.getMessageDataLength())
		return false;
	if (getMessageData() != other.getMessageData())
		return false;
	return true;
};
...
*/
t_handler_message& t_handler_message::operator=(const t_handler_message& other) {
	// check for self-assignment
	if (this != &other) {
		rawMessage.resize(HEADER_SIZE + MAX_MSG_SIZE, '\0');
		// Copy values from other to this
		rawMessage = other.getRawMessage();
		/*setDestinationBlock(other.getDestinationBlock());
		setSourceBlock(other.getDestinationBlock());
		setPriority(other.getPriority());
		setSequentialId(other.getSequentialId());
		setMessageType(other.getMessageType());
		setSourcePort(other.getSourcePort());
		setMessageDataLength(other.getMessageDataLength());
...
//template<typename T>							
//void Signal::bufferPut(T value); \\Zeinab: Implementation was moved to.h file.

void Signal::bufferPut(t_integer value)
{
	using T = t_integer;
	...
	if (inPosition == 0)
	{
		if (saveSignal)
		{
			...
			if (firstValueToBeSaved < bufferLength)
			{
				...
				{
					char *ptr = (char *)buffer;
					ptr = ptr + firstValueToBeSaved * sizeof(std::byte);
					std::ofstream fileHandler{ "./" + folderName + "/" + fileName, std::ios::out | std::ios::binary | std::ios::app };
					fileHandler.write(ptr, (bufferLength - firstValueToBeSaved) * sizeof(std::byte) * vTypeSizeOf);
					fileHandler.close();
					firstValueToBeSaved = 0;
				}
				else
...
//void Signal::bufferPut(std::byte* pValue, size_t n) \\Zeinab: Implementation was moved to.h file.

void Signal::bufferPut(t_handler_message& value) {
	t_handler_message* ptr = new(t_handler_message);

	*ptr = value;

	if ((static_cast<t_handler_message **>(buffer)[inPosition])!=nullptr)
		delete(static_cast<t_handler_message **>(buffer)[inPosition]);

	(static_cast<t_handler_message**>(buffer))[inPosition] = ptr;
	...
	if (inPosition == 0) {
		...
		if (saveSignal)
		{
			...
			if (firstValueToBeSaved < bufferLength)
			{
				std::ofstream fileHandler{ "./" + folderName + "/" + fileName, std::ios::out | std::ios::app };
				for (auto msg = firstValueToBeSaved; msg < bufferLength; msg++) {
					ptr = (static_cast<t_handler_message**>(buffer))[msg];
					fileHandler << ptr->getDestinationBlock() + "\t"
								<< ptr->getSourceBlock() + "\t"
								<< ptr->getPriority() + "\t"
								<< ptr->getSequentialId() + "\t"
								<< ptr->getMessageType() + "\t" 
								<< ptr->getSourcePort() + "\t"
								<< ptr->getMessageDataLength() + "\t" 
								<< ptr->getMessageData() + "\t" 
								<< std::endl;
				}
				fileHandler.close();
	...
	if (inPosition == 0) {
		...
		if (saveSignal)
		{
			...
			if (firstValueToBeSaved < bufferLength)
			{
				std::ofstream fileHandler{ "./" + folderName + "/" + fileName, std::ios::out | std::ios::app };
				for (auto msg = firstValueToBeSaved; msg < bufferLength; msg++) {
					ptr = (static_cast<t_message**>(buffer))[msg];
					fileHandler << ptr->getMessageType() + "\t" << ptr->getMessageDataLength() + "\t" << ptr->getMessageData() + "\t" << std::endl;
				}
				fileHandler.close();
				firstValueToBeSaved = 0;
...
//template<typename T>
//void Signal::bufferGet(T* valueAddr) \\Zeinab: Implementation was moved to.h file.
...
void Signal::bufferGet(t_handler_message* valueAddr) {

	*valueAddr = *(static_cast<t_handler_message **>(buffer)[outPosition]);
...
//template<typename T>
//void Signal::bufferRead(T* valueAddr) \\Zeinab: Implementation was moved to.h file.

void Signal::bufferRead(t_integer* valueAddr)
{
...
void Signal::bufferRead(t_message* valueAddr) {
...
void Signal::setType(signal_type sType, signal_value_type vType) { 
	...
	case signal_type::Binary:
		type = "Binary";
		break;
	case signal_type::TimeDiscreteAmplitudeDiscreteInteger:
		type = "TimeDiscreteAmplitudeDiscreteInteger";
		break;
	case signal_type::TimeContinuousAmplitudeDiscreteInteger:
		type = "TimeContinuousAmplitudeDiscreteInteger";
		break;
	...
	case signal_type::Message:
		type = "Message";
		break;
	case signal_type::HandlerMessage:
		type = "HandlerMessage";
		break;
	default:
		std::cout << "Error: netxpto_20200819.h - typeName not defined\n";
	}
};
...
void Signal::writeHeader(){
	...
	if (saveSignal && (headerType == t_header_type::fullHeader)  && (!fileName.empty())) 
	{
		...
		headerFile << "Number of Samples Per Symbol: " << getSamplesPerSymbol() << "\n";
		headerFile << "Number of Skipped Samples: " << getFirstValueToBeSaved() << "\n";
		headerFile << "Number of Samples to Be Skipped: " << getNumberOfValuesToBeSkipped() << "\n";
	...
	else if (saveSignal && (headerType == t_header_type::noHeader) && (!fileName.empty()))
	{
		headerFile.open("./" + folderName + "/" + fileName, std::ios::out);
		headerFile.close();
		headerWritten = true;
	...
	if (saveSignal && (inPosition >= firstValueToBeSaved)) {
		...
		if (!(type == "Message"))
		{
			...
			if (signalWriteMode == Signal::t_write_mode::Binary)
			{
				...
				ptr = ptr + firstValueToBeSaved * sizeof(t_binary);
				fileHandler.write((char*)ptr, (inPosition - firstValueToBeSaved) * valueTypeSizeOf());
			...
			{
				std::ofstream fileHandler{ "./" + folderName + "/" + fileName, std::ios::out | std::ios::app };
				for (t_unsigned k = 0; k < (inPosition - firstValueToBeSaved); k++)
				{
					auto vType = getValueType();
					switch (vType)
					{
...
void SuperBlock::initializeSuperBlock(void)
{
	superBlockSystem.setVerboseMode(getVerboseMode());
};

void SuperBlock::setSuperBlockSystem(std::initializer_list<Block*> blks)
{

	initializeSuperBlock();
	superBlockSystem.setSystem(blks);

	setSignalsFolderName(getSuperBlockFolderName());
	setSaveInternalSignals(getSaveInternalSignals());
...
/*			case signal_value_type::t_message:
				...
				break;*/
			default:
				std::cerr << "ERRO: netxpto_20200819.cpp (SuperBlock)" << "\n";
				return false;
			}
		}
...
//########################################################################################################################################################
 ...
 void FIR_Filter::initializeFIR_Filter(void) {
	...
	if (!getSeeBeginningOfImpulseResponse()) {
		if (getBeginningOfImpulseResponseLength() == 0)
			setBeginningOfImpulseResponseLength((int)(getImpulseResponseLength() / 2));
		outputSignals[0]->setAddToNumberOfValuesToBeSkipped(getBeginningOfImpulseResponseLength());
	}
...
void FIR_Filter::terminate(void) {
	...
	process = std::min(space, impulseResponseLength - 1);
...
System::System(std::initializer_list<Block *> Blocks, std::string signalsFolderName, std::vector<std::string> lInputParameters)
{
	...
	setSignalsFolderName(signalsFolderName);
	setLoadedInputParameters(lInputParameters);
}
...
void System::setSystem(std::initializer_list<Block *> Blocks)
{
	...
	for (int unsigned i = 0; i < SystemBlocks.size(); i++)
	{
		SystemBlocks[i]->setVerboseMode(getVerboseMode());
		SystemBlocks[i]->initializeBlock();
	}
}
...
bool System::run(std::string signalPath) {
	...
	/*2018-04-18*/
	// This version retains the directory if it already exists
	//Creates the signals folder if it doesn't exist
	//if (!std::filesystem::is_directory(signalPath) || !std::filesystem::exists(signalPath)) 
	//{
	//	std::filesystem::create_directory(signalPath);
	...
	// This version cleans the directory if it already exists
	auto numberOfDirectoriesAndFiles = std::filesystem::remove_all(signalPath);
	std::filesystem::create_directory(signalPath);
	...
	int l = 0;
	bool Alive;
	t_integer numberOfCalledBlocks{ 0 };
	if (showConsole) nviOpenConsole();
	do {
		if (showConsole) nviUpdateConsole();
		Alive = false;
		for (unsigned int i = 0; i < SystemBlocks.size(); i++) 
		{
			numberOfCalledBlocks++;
			...
			if (logValue) 
			{
				...
				snprintf(buffer, 20, "%04d-%02d-%02d %02d:%02d:%02d", 1900 + now.tm_year, now.tm_mon + 1, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec);
				logFile << "-----------------------------------------------------------------\n";
				logFile << "### " << numberOfCalledBlocks << " ### " << std::string(typeid(*SystemBlocks[i]).name()).substr(6) << " ##########" << std::endl; // Prints block name e.g. "Add"
	...
	if (showConsole) nviCloseConsole();
...
void System::setVerboseFolderName(std::string vFolderName)
{
	verboseFolderName = vFolderName;

	for (auto b : SystemBlocks) 
		b->setVerboseFolderName(vFolderName);
}

void System::setVerboseMode(t_bool vMode)
{
	verboseMode = vMode;

	for (auto b : SystemBlocks)
		b->setVerboseMode(verboseMode);
}
...
void System::openConsole()
{
	if (getConsole())
	{
		...
		ConsoleOutputCommon::clrLine(); std::cout << msgOpenConsole << std::endl;
		ConsoleOutputCommon::clrLine(); printf("#####################################################\n");
	}
}

void System::updateConsole()
{

	if (getConsole())
	{
		if (systemConsole->display != 0)
		{
			systemConsole->display--;
			return;
		}

		systemConsole->display = systemConsole->showConsolePeriodicity;

		std::map<int, std::string>::iterator it = systemConsole->getOrder.begin();

		while (it != systemConsole->getOrder.end())
		{
			std::string key = it->second;

			value_type val = systemConsole->getReturnType[key];

			ConsoleOutputCommon::clrLine();
			value_type aux = val;
			if (aux == value_type::t_int)
			{
				int intVal{ 0 };

				std::cout << key << ": " << systemConsole->getFunctionsTable[key]->nviGetFunction(&intVal);
				std::cout << std::endl;

			}
			if (aux == value_type::t_unsigned_long_int)
			{
				unsigned long int u_long_intVal{ 0 };

				std::cout << key << ": " << systemConsole->getFunctionsTable[key]->nviGetFunction(&u_long_intVal);
				std::cout << std::endl;

			}
			if (aux == value_type::t_string)
			{
				std::string strVal{ "" };

				auto str{ systemConsole->getFunctionsTable[key]->nviGetFunction(&strVal) };
				std::cout << key << ": " << str;
				std::cout << std::endl;

			}
			if (aux == value_type::t_double)
			{
				double dblVal{ 0 };

				std::cout << key << ": " << systemConsole->getFunctionsTable[key]->nviGetFunction(&dblVal);
				std::cout << std::endl;
			}
			if (aux == value_type::t_bool)
			{
				bool bVal{ false };

				bVal = systemConsole->getFunctionsTable[key]->nviGetFunction(&bVal);
...
namespace netxpto
{
	UniformRandomRealNumbers rand;
	NormalRandomRealNumbers randn;
	UniformRandomIntegerNumbers randi;
}

t_integer random(t_string distribution, t_real parameter_1)
{
	switch (netxpto::stoDistributionValue(distribution))
	{
		case netxpto::DistributionValue::poisson:
		{
			netxpto::PoissonRandomIntegerNumbers poisson(parameter_1);
			return poisson();
		}
		default:
		{
			std::cerr << "Error in the call of netxpto::random(t_string distribution, t_real parameter_1)" << std::endl;
			return 0;
		}
	}
}

netxpto::DistributionValue netxpto::stoDistributionValue(t_string str)
{
	if (str == "Poisson" || str == "poisson")
	{
		return netxpto::DistributionValue::poisson;
	}
	else
	{
		std::cerr << "Error in the call to netxpto::stoDistributonValue(t_string str)" << "\n";
		return netxpto::DistributionValue::wrong_value;
	}
}


netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers()
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s) : seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s, GeneratorValue g_value) : seed{ s }, generator_value{ g_value }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s, std::string g_value_str) : seed{ s }, generator_value{ stoGeneratorValue(g_value_str) }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(double lo, double hi) : low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s, double lo, double hi) : seed{ s }, low { lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s, GeneratorValue g_value, double lo, double hi) : seed{ s }, generator_value{ g_value }, low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomRealNumbers::UniformRandomRealNumbers(t_seed s, std::string g_value_str, double lo, double hi) : seed{ s }, generator_value{ stoGeneratorValue(g_value_str) }, low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

void netxpto::UniformRandomRealNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)
{
	generator_value = g_value;
	switch (generator_value)
	{
	case netxpto::GeneratorValue::default_random_engine:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::default_random_engine(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::minstd_rand(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand0:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::minstd_rand0(seed)));
		break;
	case netxpto::GeneratorValue::mt19937:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::mt19937(seed)));
		break;
	case netxpto::GeneratorValue::mt19937_64:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::mt19937_64(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24_base:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::ranlux24_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux48_base:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::ranlux48_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::ranlux24(seed)));
		break;
	case netxpto::GeneratorValue::knuth_b:
		r = (bind(std::uniform_real_distribution<double>(low, high), std::knuth_b(seed)));
		break;
	default:
		std::cerr << "Erro in the call of netxpto::UniformRandomRealNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)" << std::endl;
		r = (bind(std::uniform_real_distribution<double>(low, high), std::default_random_engine(seed)));
	}
}

void netxpto::UniformRandomRealNumbers::setGeneratorValue(std::string g_value_str)
{
	setGeneratorValue(stoGeneratorValue(g_value_str));
}

netxpto::NormalRandomRealNumbers::NormalRandomRealNumbers()
{
	setGeneratorValue(generator_value);
}

netxpto::NormalRandomRealNumbers::NormalRandomRealNumbers(t_seed s) : seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::NormalRandomRealNumbers::NormalRandomRealNumbers(t_seed s, GeneratorValue g_value) : seed{ s }, generator_value{ g_value }
{
	setGeneratorValue(generator_value);
}

netxpto::NormalRandomRealNumbers::NormalRandomRealNumbers(t_seed s, std::string g_value_str) : seed{ s }, generator_value{ stoGeneratorValue(g_value_str) }
{
	setGeneratorValue(generator_value);
}

void netxpto::NormalRandomRealNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)
{
	generator_value = g_value;
	switch (generator_value)
	{
	case netxpto::GeneratorValue::default_random_engine:
		r = (bind(std::normal_distribution<double>(), std::default_random_engine(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand:
		r = (bind(std::normal_distribution<double>(), std::minstd_rand(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand0:
		r = (bind(std::normal_distribution<double>(), std::minstd_rand0(seed)));
		break;
	case netxpto::GeneratorValue::mt19937:
		r = (bind(std::normal_distribution<double>(), std::mt19937(seed)));
		break;
	case netxpto::GeneratorValue::mt19937_64:
		r = (bind(std::normal_distribution<double>(), std::mt19937_64(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24_base:
		r = (bind(std::normal_distribution<double>(), std::ranlux24_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux48_base:
		r = (bind(std::normal_distribution<double>(), std::ranlux48_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24:
		r = (bind(std::normal_distribution<double>(), std::ranlux24(seed)));
		break;
	case netxpto::GeneratorValue::knuth_b:
		r = (bind(std::normal_distribution<double>(), std::knuth_b(seed)));
		break;
	default:
		std::cerr << "Erro in the call of netxpto::NormalRandomRealNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)" << std::endl;
		r = (bind(std::normal_distribution<>(), std::default_random_engine(seed)));
	}
}

void netxpto::NormalRandomRealNumbers::setGeneratorValue(std::string g_value_str)
{
	setGeneratorValue(stoGeneratorValue(g_value_str));
}

netxpto::PoissonRandomIntegerNumbers::PoissonRandomIntegerNumbers(t_real lamb) : lambda{ lamb }
{
	setGeneratorValue(generator_value);
}

netxpto::PoissonRandomIntegerNumbers::PoissonRandomIntegerNumbers(t_real lamb, t_seed s) : lambda{ lamb }, seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::PoissonRandomIntegerNumbers::PoissonRandomIntegerNumbers(t_real lamb, t_seed s, GeneratorValue g_value) : lambda{ lamb }, seed{ s }, generator_value{ g_value }
{
	setGeneratorValue(generator_value);
}

netxpto::PoissonRandomIntegerNumbers::PoissonRandomIntegerNumbers(t_real lamb, t_seed s, std::string g_value_str) : lambda{ lamb }, seed{ s }, generator_value{ stoGeneratorValue(g_value_str) }
{
	setGeneratorValue(generator_value);
}

void netxpto::PoissonRandomIntegerNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)
{
	generator_value = g_value;
	switch (generator_value)
	{
	case netxpto::GeneratorValue::default_random_engine:
		r = (bind(std::poisson_distribution<>(lambda), std::default_random_engine(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand:
		r = (bind(std::poisson_distribution<>(lambda), std::minstd_rand(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand0:
		r = (bind(std::poisson_distribution<>(lambda), std::minstd_rand0(seed)));
		break;
	case netxpto::GeneratorValue::mt19937:
		r = (bind(std::poisson_distribution<>(lambda), std::mt19937(seed)));
		break;
	case netxpto::GeneratorValue::mt19937_64:
		r = (bind(std::poisson_distribution<>(lambda), std::mt19937_64(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24_base:
		r = (bind(std::poisson_distribution<>(lambda), std::ranlux24_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux48_base:
		r = (bind(std::poisson_distribution<>(lambda), std::ranlux48_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24:
		r = (bind(std::poisson_distribution<>(lambda), std::ranlux24(seed)));
		break;
	case netxpto::GeneratorValue::knuth_b:
		r = (bind(std::poisson_distribution<>(lambda), std::knuth_b(seed)));
		break;
	default:
		std::cerr << "Erro in the call of netxpto::PoissonRandomIntegerNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)" << std::endl;
		r = (bind(std::poisson_distribution<>(lambda), std::default_random_engine(seed)));
	}
}

void netxpto::PoissonRandomIntegerNumbers::setGeneratorValue(std::string g_value_str)
{
	setGeneratorValue(stoGeneratorValue(g_value_str));
}


netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers()
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s) : seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s, GeneratorValue g_value) : generator_value{ g_value }, seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s, std::string g_value_str) : generator_value{ stoGeneratorValue(g_value_str) }, seed{ s }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(int lo, int hi) : low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}


netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s, int lo, int hi) : seed{ s }, low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s, GeneratorValue g_value, int lo, int hi) : generator_value{ g_value }, seed{ s }, low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}

netxpto::UniformRandomIntegerNumbers::UniformRandomIntegerNumbers(t_seed s, std::string g_value_str, int lo, int hi) : generator_value{ stoGeneratorValue(g_value_str) }, seed{ s }, low{ lo }, high{ hi }
{
	setGeneratorValue(generator_value);
}


void netxpto::UniformRandomIntegerNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)
{
	generator_value = g_value;
	switch (generator_value)
	{
	case netxpto::GeneratorValue::default_random_engine:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::default_random_engine(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::minstd_rand(seed)));
		break;
	case netxpto::GeneratorValue::minstd_rand0:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::minstd_rand0(seed)));
		break;
	case netxpto::GeneratorValue::mt19937:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::mt19937(seed)));
		break;
	case netxpto::GeneratorValue::mt19937_64:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::mt19937_64(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24_base:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::ranlux24_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux48_base:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::ranlux48_base(seed)));
		break;
	case netxpto::GeneratorValue::ranlux24:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::ranlux24(seed)));
		break;
	case netxpto::GeneratorValue::knuth_b:
		r = (bind(std::uniform_int_distribution<int>(low, high), std::knuth_b(seed)));
		break;
	default:
		std::cerr << "Erro in the call of netxpto::UniformRandomIntegerNumbers::setGeneratorValue(netxpto::GeneratorValue g_value)" << std::endl;
		r = (bind(std::uniform_int_distribution<int>(low, high), std::default_random_engine(seed)));
	}
}

netxpto::GeneratorValue netxpto::stoGeneratorValue(std::string str)
{
	if (str == "netxpto::default_random_engine" || str == "default_random_engine")
	{
		return netxpto::GeneratorValue::default_random_engine;
	}
	else if (str == "netxpto::minstd_rand" || str == "minstd_rand")
	{
		return netxpto::GeneratorValue::minstd_rand;
	}
	else if (str == "netxpto::minstd_rand0 " || str == "minstd_rand0 ")
	{
		return netxpto::GeneratorValue::minstd_rand0;
	}
	else if (str == "netxpto::mt19937" || str == "mt19937")
	{
		return netxpto::GeneratorValue::mt19937;
	}
	else if (str == "netxpto::mt19937_64" || str == "mt19937_64")
	{
		return netxpto::GeneratorValue::mt19937_64;
	}
	else if (str == "netxpto::ranlux24_base" || str == "ranlux24_base")
	{
		return netxpto::GeneratorValue::ranlux24_base;
	}
	else if (str == "netxpto::ranlux24" || str == "ranlux24")
	{
		return netxpto::GeneratorValue::ranlux24;
	}
	else if (str == "netxpto::knuth_b" || str == "knuth_b")
	{
		return netxpto::GeneratorValue::knuth_b;
	}
	else
	{
		std::cerr << "Error in the call to netxpto::stoGeneratorValue(std::string str)" << "\n";
		return netxpto::GeneratorValue::wrong_value;
	}
}

void netxpto::rng(t_seed s)
{
	rand.setSeed(s);
	randn.setSeed(s);
	randi.setSeed(s);
...
void netxpto::rng(t_seed s, netxpto::GeneratorValue g_value)
{
	rand.setSeed(s,g_value);
	randn.setSeed(s, g_value);
	randi.setSeed(s, g_value);
}

void netxpto::rng(t_seed s, std::string g_value_str)
{
	rng(s, stoGeneratorValue(g_value_str));
}

void netxpto::rng(std::string s_str)
{
	if (s_str == "shuffle")
		rng((t_seed)(std::chrono::system_clock::now().time_since_epoch().count()));
	else if (s_str == "random_device")
		rng((t_seed)(std::random_device{}()));
	else
		std::cerr << "Error in the call to the netxpto::rng() function" << "\n";
}

void netxpto::rng(std::string s_str, netxpto::GeneratorValue g_value)
{
	if (s_str == "shuffle")
	{
		rng((t_seed)(std::chrono::system_clock::now().time_since_epoch().count()), g_value);
	}
	else if (s_str == "random_device")
		rng((t_seed)(std::random_device{}()), g_value);
	else
	{
		std::cerr << "Error in the call to the netxpto::rng(std::string shuffle, netxpto::GeneratorValue generator) function" << "\n";
		return;
	}
}

void netxpto::rng(std::string s_str, std::string g_value_str)
{
	rng(s_str, stoGeneratorValue(g_value_str));
}
...
void Fft::Radix2(std::vector<double> &real, vector<double> &imag, int s)
{
	...
	for (size_t i = 0; i < n / 2; i++)
	{
		cosTable[i] = cos(-s*2 * PI * i / n);
		sinTable[i] = sin(-s*2 * PI * i / n);
	}
...
void Fft::transformBluestein(std::vector<double> &real, vector<double> &imag) 
{
	...
	for (size_t i = 0; i < n; i++) {
		double temp = PI * (size_t)((unsigned long long)i * i % ((unsigned long long)n * 2)) / n;
		// Less accurate version if long long is unavailable: double temp = PI * i * i / n;
		cosTable[i] = cos(temp);
		sinTable[i] = sin(temp);
	}
...
{
	if (inputParametersFileName == "")
	{
		std::cerr << "ERROR: Empty inputParametersFileName ";
		exit(1);
	}
	std::string str{ "./" + inputParametersFileName };
			...
			if (std::string(line).substr(0, 2) != "//") { //Lines that start by // are comments
				...
				if (parameters.find(splitline[0]) != parameters.end()) { //if parameter exists
					if (parameters[splitline[0]]->getType() == INT) //If parameter is an int
 					...
 					else if (parameters[splitline[0]]->getType() == LONG)
						parameters[splitline[0]]->setValue(parseLong(splitline[1]));
...
void SystemInputParameters::addInputParameter(std::string name, long* variable)
{
	parameters[name] = new Parameter(variable);
}
...
long SystemInputParameters::parseLong(std::string str)
{
...
void SystemInputParameters::Parameter::setValue(long value)
{
	if (type != LONG) throw std::invalid_argument("Parameter is not of type LONG");
	*l = value;
}
...
SystemInputParameters::Parameter::Parameter(long* elem)
{
	type = LONG;
	l = elem;
}
...
std::vector<double> getWindow(WindowType windowType, int windowSize) {
	...
	case WindowType::Hamming:
		for (int x = 0; x < windowSize; x++) {
			wn[x] = 0.54 - 0.46*cos(2 * PI*x / (((double) windowSize) - 1));
		}
		return wn;
