#include "dv_qkd_error_correction_ldpc_20200819.h"

void ErrorCorrection::initialize(void) 
{
	if (errorCorrectionMethod == ErrorCorrection::t_method::Cascade)
	{
	...
	else if (errorCorrectionMethod == ErrorCorrection::t_method::LDPC)
	{
		// codeLength = n; codeRank = k
		// we are assuming a (n,k) = (1944, 972) code , (n-k) x n = 972 x 1944 = 1889568, with only 6885 non-zero values
		sparseParityCheckMatrix.resize(codeLength-codeRank, codeLength);
		sparseParityCheckMatrix.reserve(numberOfNonZerosValuesInParityCheckMatrix);
		std::fstream fp;
		fp.open("sparseParityCheckMatrix.txt", std::ios::in);
		double data;
		int row{ 0 }, column{ 0 };
		while (true)
		{
;			fp >> data;
		...
		receivedData.resize(codeLength, 1);

		sindrome.resize(codeLength - codeRank, 1);
		sindromeSize = sindrome.size();
	}
}
...
bool ErrorCorrection::runBlock(void)
{
	...
	if (errorCorrectionMethod == ErrorCorrection::t_method::Cascade)
	{
		...
		if (errorCorrectionRole == ErrorCorrection::t_role::Tx) 
		{
	...
	//			int spaceEC = outputSignals[1]->space();
		...
		else if (errorCorrectionRole == ErrorCorrection::t_role::Rx) {
	...
	else if (errorCorrectionMethod==ErrorCorrection::t_method::LDPC)
	{
		...
		if (sindrome_posOut > 0)        // There are some values of the sindrome that need to be send
		{
			auto space = outputSignals[1]->space();
			auto process = std::min(space, (t_integer)(sindromeSize - sindrome_posOut));
			alive = (t_bool)process;
			for (auto k = sindrome_posOut; k < process; k++)
				outputSignals[1]->bufferPut((t_binary) (sindrome[k]));

			sindrome_posOut = (sindrome_posOut + process) % (sindromeSize);
		}

		auto ready = inputSignals[0]->ready();
		auto process = std::min((Eigen::Index) ready, codeLength - codeRank - posIn);
		alive = (t_bool)process;

		for (auto k = posIn; k < process; k++)
		{
			t_binary dataIn;
			inputSignals[0]->bufferGet(&dataIn);
			receivedData(k, 0) = dataIn;
		}
		posIn = posIn + process;

		if (posIn == codeLength - codeRank)
		{
			if (errorCorrectionRole == ErrorCorrection::t_role::Rx)
			{
				// Calculates the sindrome and send it to the Tx, s(1 x (n-k)) = r(1 x n) H^T(n x (n-k))

				if (sindrome_posOut == 0)
				{
					sindrome = sparseParityCheckMatrix * receivedData;
					sindrome = sindrome.unaryExpr([](const t_binary x) {return x % 2;  }); //to compute mod(sindrome,2)

					std::ofstream file;
					file.open("sindrome_rx.txt");

					t_binary aux{ 0 };
					for (auto k = 0; k < sindromeSize; k++)
						file << std::to_string(sindrome[k]);
			...
			else if (errorCorrectionRole == ErrorCorrection::t_role::Tx)
			{
				sindrome = sparseParityCheckMatrix * receivedData;
				sindrome = sindrome.unaryExpr([](const t_binary x) {return x % 2;  }); //to compute mod(sindrome,2)

				std::ofstream file;
				file.open("sindrome_tx.txt");
				...
				// Compares the calculated sindrome with the received sindrome
				// s(1 x k) = d0(1 x n) H(n x k) -> s(1 x k) = d1(1 x n) H(n x k) -> ... 

				// If when the maximum number of iterations is reached and sindromes are still different discard the data otherwise keep the data

				// Signals Bob about it decision (discard or keep)
			}
			else
			{
				std::cerr << "Undefined behavior on the error correction block_2.";
			}
		}
	...
	{
		std::cerr << "Undefined behavior on the error correction block_3.";
	}
