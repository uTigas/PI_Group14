    ...
    const fetchOrganization = async () => {
        try {
            ...
            if (response) {
                ...
                setTitle(response.data.organization.name); // Set the title with organization's name
            } else {
                setVaults([]);
                setMembers([]);
                setRole("");
                ...
                setTitle("Organization"); // Reset title if no organization is found
            }
        } catch (error) {
                        ...
                        <IonCol>
                            ...
                            {members.length !== 0 ? (
                                members.map((item, index) => (
                                    <IonItem key={item.username}>
                                        <IonCard className="card organization-card ion-padding" style={{ width: "100%" }}>
                                            <IonCardHeader>
                                            ...
                                            <IonCardContent>
                                                <strong></strong>
                                                <IonText style={{ fontWeight: "bold", fontStyle: "italic" }}>Joined in {format(new Date(item.joined), "dd-MM-yyyy")}</IonText>
                                            </IonCardContent>
                                        </IonCard>
                                    </IonItem>
                                ))
