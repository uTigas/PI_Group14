...
void DvQkdRx_LDPC::initialize(void) 
{
	...
	// Configure blocks
	CvQokdLdpcRxSindromeReconciliation_.setParityCheckMatrixFolderName(parityCheckMatrixFolderName);
	CvQokdLdpcRxSindromeReconciliation_.setParityCheckMatrixFileName(parityCheckMatrixFileName);
	...
	setSuperBlockSystem(
		{
			...
			&CvQokdLdpcRxSindromeReconciliation_,
...
bool DvQkdRx_LDPC::runBlock(void) 
{
	...
	if ((dispBitsTracker >= dispBitsInterval) && (toPrint)) {
		...
		double ecBitsIn = (double)CvQokdLdpcRxSindromeReconciliation_.getTotalNumberOfInputBits();
		double ecBitsOut = (double)CvQokdLdpcRxSindromeReconciliation_.getTotalNumberOfOutputBits();
