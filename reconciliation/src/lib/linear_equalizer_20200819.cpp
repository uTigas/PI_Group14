...
void LinearEqualizer::initialize(void)
{
	...
	outputSignals[1]->setNumberOfValuesToBeSkipped(inputSignals[1]->getNumberOfValuesToBeSkipped() + 2 * positionIn); //why the two? because it works..., ANP, 18/12/20220
}
...
bool LinearEqualizer::runBlock(void)
{
	...
	for (auto k = 0; k < process; k++)
	{
		...
		signalTimeDomain[positionIn] = std::complex(re,im);
		positionIn++;
	}
	...
	if ((positionIn == overlapSaveBlockLength) && (positionOut == -1))
	{
		...
		if (fiberLength_m > 0) // implements the linear operator
		{
	...
	if (positionOut > -1)
	{
		...
		{
			outputSignals[0]->bufferPut(real(signalTimeDomain[positionOut]));
			outputSignals[1]->bufferPut(imag(signalTimeDomain[positionOut]));
		}
...
t_real LinearEqualizer::saveRealVector(std::vector<t_real>& data, std::string str, int begin, int end)
{
	std::ofstream outRef;
...
t_real LinearEqualizer::saveComplexVector(std::vector<t_complex>& data, std::string str, int begin, int end)
{
	std::ofstream outRef;
	outRef.open(getSignalsFolderName() + "/" + str, std::ios::app);
	outRef.precision(15);
