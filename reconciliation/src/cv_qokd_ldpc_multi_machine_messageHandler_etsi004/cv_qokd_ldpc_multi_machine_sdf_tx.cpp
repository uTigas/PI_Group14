...
//		LoadEtsi004_Tx.setAsciiFileName("tx_raw_done");
...
//		LoadEtsi004_Tx.setAsciiFileNameTailNumberModulos(param.asciiFileNameTailNumberModulos);
		...
		*/
		...
		SaveAscii_Tx.setAsciiFileName("sym_tx");
		SaveAscii_Tx.setAsciiFileNameTailNumber("0");
		SaveAscii_Tx.setAsciiFileNameExtension("b64");
		SaveAscii_Tx.setAsciiFileNameTailNumberModulos(0);

		Binary _{"_.sgn", 36, hType, sWriteMode};
        ConvertB64 b64_converter{{&Key_Tx, &_},{&Key_base64TX, &_}};
...
//		//////
		...
		DvQkdLdpcTxMessageProcessorTransmitter DvQkdLdpcTxMessageProcessorTransmitterLoadFile_Tx{ {&BasesAckToRx_Tx, &ParameterEstimation_SeedToRx_Tx, &ParameterEstimation_RatioToRx_Tx, &ParameterEstimation_NumberOfBitsPerEstimationBlockToRx_Tx, &ParameterEstimation_DataToRx_Tx,  &SindromeToRx_Tx, &HashToRx_Tx}, {&MessagesToLoadFile_Tx_} };

		IPTunnel IPTunnel_Client_Tx_LoadFile{ {&MessagesToLoadFile_Tx_}, {} }; 		// From Tx (App) to Rx (LoadFile) 
		...
		IPTunnel_Client_Tx_LoadFile.setRemoteMachinePort(param_LoadFile.txReceivingPort);
		IPTunnel_Client_Tx_LoadFile.setVerboseMode(param_LoadFile.verboseMode);


		IPTunnel IPTunnel_Server_Tx_LoadFile{ {}, {&MessagesFromLoadFile_Tx_} };	// To Tx (App) from Rx (LoadFile)
		...
		IPTunnel_Server_Tx_LoadFile.setLocalMachinePort(param_LoadFile.rxReceivingPort);
		IPTunnel_Server_Tx_LoadFile.setVerboseMode(param_LoadFile.verboseMode);
		
		DvQkdLdpcTxMessageProcessorReceiver DvQkdLdpcTxMessageProcessorReceiverLoadFile_Tx{ {&MessagesFromLoadFile_Tx_}, {&BasesFromRx_Tx, &ParameterEstimation_SeedFromRx_Tx, &ParameterEstimation_RatioFromRx_Tx, &ParameterEstimation_NumberOfBitsPerEstimationBlockFromRx_Tx, &ParameterEstimation_DataFromRx_Tx, &SindromeFromRx_Tx, &PermutationsFromRx_Tx, &ToggleBERChange_Tx} };
