...
//		param.readSystemInputParameters();
		...
		Binary Raw_Tx{ "S00_Raw_Key_Rx.sgn", (t_unsigned_long) param_LoadFile.buffSize, hType, sWriteMode };
		...
		Binary Key_Tx{ "S100_Raw_Key_Rx.sgn", (t_unsigned_long) param_LoadFile.buffSize };
		...
		Message MessagesToLoadFile_Rx{ "S17_MessagesToRx.sgn", 10, hType, sWriteMode };
		HandlerMessage MessagesToLoadFile_Rx_{ "S17_MessagesToRx.sgn", 10, hType, sWriteMode };

		Message MessagesFromLoadFile_Rx{ "S18_Tx_MessagesFromRx.sgn", 10, hType, sWriteMode };
		HandlerMessage MessagesFromLoadFile_Rx_{ "S18_Tx_MessagesFromRx.sgn", 10, hType, sWriteMode };

		// #####################################################################################################
		// ########################### Tx Blocks Declaration and Inicialization ###################################
		// #####################################################################################################

		LoadEtsi004 LoadEtsi004_Rx{ {},{&Raw_Tx} };
		...
		LoadEtsi004_Rx.setAsciiFileNameTailNumberModulos(param_LoadFile.asciiFileNameTailNumberModulos);
...
//		CvQokdLdpcTxSindromeReconciliation_Tx.setKeyType(param.keyType);
		...
		// Oblivious keys
//		SaveAscii SaveAscii_Rx{ {&Key_Tx}, {} };
//	    //SaveAscii_Tx.setAsciiFileName("tx_key");
//		SaveAscii_Rx.setAsciiFileName("rx_raw_input");
...
//        	ittTxTransmitter};
		...
		dttRxReceiverLoadFile.add("Msg_Tx_LoadFile", 0);
		MessageHandler DvQkdLdpcTxMessageProcessorReceiverLoadFile_Rx_{ 
			{&MessagesFromLoadFile_Rx}, 
			{&MessagesFromLoadFile_Rx_},
			dttRxReceiverLoadFile,
        	FUNCTIONING_AS_RX,
		};

		InputTranslationTable ittRxTransmitterLoadFile;
		ittRxTransmitterLoadFile.add(0, {"Msg_Rx_LoadFile", "Msg_Tx_LoadFile"});
...
//		DvQkdLdpcTxMessageProcessorReceiver DvQkdLdpcTxMessageProcessorReceiver_Tx{ {&MessagesFromRx_Tx}, {&BasesFromRx_Tx, &ParameterEstimation_SeedFromRx_Tx, &ParameterEstimation_RatioFromRx_Tx, &ParameterEstimation_NumberOfBitsPerEstimationBlockFromRx_Tx, &ParameterEstimation_DataFromRx_Tx, &SindromeFromRx_Tx, &PermutationsFromRx_Tx, &ToggleBERChange_Tx} };
		...
		DvQkdLdpcTxMessageProcessorTransmitter DvQkdLdpcTxMessageProcessorTransmitterLoadFile_Rx{ {&BasesAckToRx_Tx, &ParameterEstimation_SeedToRx_Tx, &ParameterEstimation_RatioToRx_Tx, &ParameterEstimation_NumberOfBitsPerEstimationBlockToRx_Tx, &ParameterEstimation_DataToRx_Tx,  &SindromeToRx_Tx, &HashToRx_Tx}, {&MessagesToLoadFile_Rx} };
//		DvQkdLdpcTxMessageProcessorTransmitter DvQkdLdpcTxMessageProcessorTransmitterLoadFile_Tx{ {}, {&MessagesToLoadFile_Tx} };


		IPTunnel IPTunnel_Client_Rx_LoadFile{ {&MessagesToLoadFile_Rx_}, {} };		// From Tx (LoadFile) to App (Rx)
		IPTunnel_Client_Rx_LoadFile.setLocalMachineIpAddress(param_LoadFile.txIpAddress);
		IPTunnel_Client_Rx_LoadFile.setRemoteMachineIpAddress(param_LoadFile.rxIpAddress);
		IPTunnel_Client_Rx_LoadFile.setRemoteMachinePort(param_LoadFile.rxReceivingPort);
		IPTunnel_Client_Rx_LoadFile.setVerboseMode(param_LoadFile.verboseMode);

		IPTunnel IPTunnel_Server_Rx_LoadFile{ {}, {&MessagesFromLoadFile_Rx_} };	// To Tx (LoadFile) from Rx (App)
		IPTunnel_Server_Rx_LoadFile.setLocalMachineIpAddress(param_LoadFile.txIpAddress);
		IPTunnel_Server_Rx_LoadFile.setRemoteMachineIpAddress(param_LoadFile.rxIpAddress);
		IPTunnel_Server_Rx_LoadFile.setLocalMachinePort(param_LoadFile.txReceivingPort);
		IPTunnel_Server_Rx_LoadFile.setVerboseMode(param_LoadFile.verboseMode);
		
		DvQkdLdpcTxMessageProcessorReceiver DvQkdLdpcTxMessageProcessorReceiverLoadFile_Rx{ {&MessagesFromLoadFile_Rx}, {&BasesFromRx_Tx, &ParameterEstimation_SeedFromRx_Tx, &ParameterEstimation_RatioFromRx_Tx, &ParameterEstimation_NumberOfBitsPerEstimationBlockFromRx_Tx, &ParameterEstimation_DataFromRx_Tx, &SindromeFromRx_Tx, &PermutationsFromRx_Tx, &ToggleBERChange_Tx} };
...
//				&DvQkdLdpcTxMessageProcessorReceiver_Tx_,
		...
		BlockGetFunction<std::_Mem_fn<std::string(IPTunnel::*)() const>, IPTunnel, std::string> Local_Machine_Address_{ &IPTunnel_Server_Rx_LoadFile, std::mem_fn(&IPTunnel::getLocalMachineIpAddress) };
		Console_.addGetFunction("Local Machine Address", &Local_Machine_Address_, value_type::t_string);

		BlockGetFunction<std::_Mem_fn<int (IPTunnel::*)() const>, IPTunnel, int> Local_Machine_Port_{ &IPTunnel_Server_Rx_LoadFile, std::mem_fn(&IPTunnel::getLocalMachinePort) };
		Console_.addGetFunction("Local Machine Receiving Port", &Local_Machine_Port_, value_type::t_int);

		BlockGetFunction<std::_Mem_fn<std::string(IPTunnel::*)() const>, IPTunnel, std::string> Remote_Machine_Address_{ &IPTunnel_Client_Rx_LoadFile, std::mem_fn(&IPTunnel::getRemoteMachineIpAddress) };
		Console_.addGetFunction("Remote Machine Address", &Remote_Machine_Address_, value_type::t_string);

		BlockGetFunction<std::_Mem_fn<int (IPTunnel::*)() const>, IPTunnel, int> Remote_Machine_Port_{ &IPTunnel_Client_Rx_LoadFile, std::mem_fn(&IPTunnel::getRemoteMachinePort) };
...
//		Console_.addGetFunction("Tx: Percentage Of Discarded Pairs", &PercentageOfDiscardedPairs_Tx, value_type::t_double);
		...
		MainSystem_.setMsgOpenConsole("Rx Load File System Console");
